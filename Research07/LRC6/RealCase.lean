...
nonzero; the (n−1)-runner rational hypothesis (`lrc5`, i.e. `lrc5_rat_finset`)
puts `t·w` in the open cube `(δ,1−δ)ⁿ` with `δ ∈ (1/6, 1/5)`;
`orbit_dense_annihilator` approximates by an orbit point.

This is the dimension-5 port of `M3/BHK.lean`'s
`lrc5_real_of_irrational_ratio` (itself BHK Lemma 8 at n = 5). BHK state the
reduction explicitly for n = 6 (bhk §4): the irrational case of Conjecture 1
for n = 6 follows from the rational case for n = 5.
-/

noncomputable section

/-- **BHK Lemma 8 instantiated at n = 6.** If every ≤4-element set of positive
rationals admits a `1/5`-lonely time, then any 5 positive real speeds with an
irrational ratio admit a `t` with all `circ (t·uᵢ) > 1/6` (strict — slack from
`δ ∈ (1/6, 1/5)`). -/
theorem lrc6_real_of_irrational_ratio
    (hlrc5 : ∀ S : Finset ℚ, (∀ q ∈ S, 0 < q) → S.card ≤ 4 →
      ∃ t : ℝ, 0 < t ∧ ∀ q ∈ S, (1 / 5 : ℝ) ≤ circ (t * q))
    {u : Fin 5 → ℝ} (hpos : ∀ i, 0 < u i)
    (hirr : ¬ ∃ c : ℝ, ∀ i, ∃ q : ℚ, u i = c * q) :
    ∃ t : ℝ, 0 < t ∧ ∀ i, (1 / 6 : ℝ) < circ (t * u i) := by
  ...
  have hadj : ∀ k : Fin 5, s k / r k ≤ s i / r i ∨ s j / r j ≤ s k / r k := by
    intro k
    ...
    calc (Finset.univ.image fun k => |w k|).card
      ...
      _ = 4 := by
          rw [Finset.card_erase_of_mem (Finset.mem_univ i), Finset.card_univ,
            Fintype.card_fin]
  obtain ⟨t, htpos, ht⟩ := hlrc5 _ hSpos hScard
  -- `circ (t·wₖ) ≥ 1/5` for every `k` (`t > 0`, `circ` ignores the sign).
  have htk : ∀ k : Fin 5, (1 / 5 : ℝ) ≤ circ (t * (w k : ℝ)) := by
    intro k
    ...
    exact h
  -- (v) `δ = 11/60 ∈ (1/6, 1/5)` and `y := i ↦ t·wᵢ ∈ T` lies in the annihilator.
  set δ : ℝ := 11 / 60 with hδdef
  have hδlt : δ < 1 / 5 := by norm_num [hδdef]
  have hδgt : (1 / 6 : ℝ) < δ := by norm_num [hδdef]
  have hδpos : 0 < δ := by norm_num [hδdef]
  have hcirc : ∀ k : Fin 5, δ < circ (t * (w k : ℝ)) := fun k => hδlt.trans_le (htk k)
  ...
  have hUopen : IsOpen {y : Fin 5 → UnitAddCircle | ∀ i, δ < ‖y i‖} := by
    have heq : {y : Fin 5 → UnitAddCircle | ∀ i, δ < ‖y i‖} =
        ⋂ i : Fin 5, {y : Fin 5 → UnitAddCircle | δ < ‖y i‖} := by
      ext y
    ...
    exact isOpen_iInter_of_finite fun i =>
      isOpen_lt continuous_const
        ((continuous_apply i).norm :
          Continuous fun y : Fin 5 → UnitAddCircle => ‖y i‖)
  have hyU : (fun i => ((t * (w i : ℝ) : ℝ) : UnitAddCircle)) ∈
      {y : Fin 5 → UnitAddCircle | ∀ i, δ < ‖y i‖} := fun i => hcirc i
  obtain ⟨z, hzI⟩ := (mem_closure_iff.mp (orbit_dense_annihilator u hy_ann) _
    hUopen hyU)
  obtain ⟨hzU, hzr⟩ := hzI
  obtain ⟨t', ht'⟩ := hzr
  have hδt : ∀ i : Fin 5, δ < circ (t' * u i) := by
    intro i
    have hzi := hzU i
    rw [← ht'] at hzi
    exact hzi
